         ...
         for(modulo m: c.getModulo()) {
        	 boolean alumnopresente=m.SearchAlumno(alumno);
      	     if(m.getName()==Modulo &&  alumnopresente==true) {
      	    	 ...
      	    	 System.out.println("El  alumno no se ha podido desmatricular,  "+alumno.getName()+" no está matriculado la asignatura "+m.getName()+".");
      	     }
         }
	  }
}
...
public void AlumnosMatriculadosModulo(String Modulo) {
	  ...
	  for(curso c:cursos) {
       ...
       for(modulo m: c.getModulo()) {
      	 System.out.println("Entré"+m.getName()+"|"+Modulo.toUpperCase());
